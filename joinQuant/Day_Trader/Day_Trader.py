...
#
#   一、指数：
#       1、 创业板指（399006） 创业板股票（399006，剔除上市不满130日创业板股票）
#       2、 中小板指（399005） 小市值（流通市值小于100亿，与现有的小市值策略一致）
#       3、 沪深300（399300） 买入510310（回测时按这个ETF来做，但实际上可能要变成买所有的股票，这个成交量太小了）
#       4、 上证50（000016） 买入510050 
...
#       3、1个指数向上，3成仓
#       4、都向下，空仓
#       备注：指数向上指20日均线斜率>1.01 

#   四、选股:（调仓时判断）
#       1、 选择斜率最高的指数对于的股票标的
#       2、 流通按市值排序（小市值优先） 
#       3、仓位调整不是每天调整，等调整周期到的时候再调整。
...
def after_code_changed(context):
    ...
    def check_chang(rules, config):
        ...
        for c in config:
            ...
            find_old = None
            for old_r in rules:
                if old_r.__class__ == c[
                        g.cs_class_name] and old_r.name == c[
                        g.cs_name]:
                    find_old = old_r
                    break
    ...
    # 重整所有规则
    g.position_stock_rules = check_chang(
        g.position_stock_rules, g.position_stock_config)
    g.adjust_condition_rules = check_chang(
        g.adjust_condition_rules, g.adjust_condition_config)
    g.pick_stock_by_query_rules = check_chang(
        g.pick_stock_by_query_rules,
     g.pick_stock_by_query_config)
    g.filter_stock_list_rules = check_chang(
        g.filter_stock_list_rules,
     g.filter_stock_list_config)
    g.adjust_position_rules = check_chang(
        g.adjust_position_rules, g.adjust_position_config)
...
class Rule(object):
    ...
    def get_obj_by_class_type(self, class_type):
        if self.on_get_obj_by_class_type is not None:
...
class Index28_condition(Adjust_condition):
    ...
    def update_params(self, context, params):
        ...
        self.index_growth_rate = params.get(
            'index_growth_rate', self.index_growth_rate)
    ...
    def handle_data(self, context, data):
        ...
        self.log_info(
            "当前%s指数的20日涨幅 [%.2f%%]" %
            (get_security_info(
                self.index2).display_name,
                gr_index2 *
                100))
        self.log_info(
            "当前%s指数的20日涨幅 [%.2f%%]" %
            (get_security_info(
                self.index8).display_name,
                gr_index8 *
                100))
...
class Filter_limite(Filter_query):
    ...
    def update_params(self, context, params):
        self.pick_stock_count = params.get(
            'pick_stock_count', self.pick_stock_count)
...
class Filter_limitup(Filter_stock_list):
    ...
    def filter(self, context, data, stock_list):
        threshold = 1.00
        return [stock for stock in stock_list if data[stock].close < data[stock].high_limit * threshold]
...
class Filter_limitdown(Filter_stock_list):
    ...
    def filter(self, context, data, stock_list):
        ...
        if (len(context.portfolio.positions) == 0):
            ...
            return [stock for stock in stock_list if data[stock].close > data[stock].low_limit * threshold]
    ...
    def handle_data(self, context, data):
        for stock in context.portfolio.positions.keys():
            ...
            if cur_price < self.last_high[stock] * (1 - threshold):
                self.log_info(
                    "==> 个股止损, stock: %s, cur_price: %f, last_high: %f, threshold: %f" %
    ...
    def handle_data(self, context, data):
        for stock in context.portfolio.positions.keys():
            ...
            if cur_price > position.avg_cost * (1 + threshold):
                self.log_info(
                    "==> 个股止盈, stock: %s, cur_price: %f, avg_cost: %f, threshold: %f" %
                    (stock, cur_price, self.last_high[stock], threshold))
...
class Stop_loss_by_price(Adjust_condition):
    ...
    def handle_data(self, context, data):
        ...
        if not self.is_day_stop_loss_by_price:
            h = attribute_history(
                self.index, self.day_count, unit='1d', fields=(
                    'close', 'high', 'low'), skip_paused=True)
            low_price_130 = h.low.min()
            high_price_130 = h.high.max()
            if high_price_130 > self.multiple * low_price_130 and h['close'][-1] < h[
                    'close'][-4] * 1 and h['close'][-1] > h['close'][-100]:
                # 当日第一次输出日志
                self.log_info(
                    "==> 大盘止损，%s指数前130日内最高价超过最低价2倍, 最高价: %f, 最低价: %f" %
                    (get_security_info(
                        self.index).display_name,
                        high_price_130,
                        low_price_130))
...
class Stop_loss_by_growth_rate(Adjust_condition):
    ...
    def handle_data(self, context, data):
        ...
        if cur_growth_rate < self.stop_loss_growth_rate:
            self.log_warn(
                '当日涨幅 [%s : %.2f%%] 低于阀值 %.2f%%,清仓止损!' %
                (self.index, cur_growth_rate * 100, self.stop_loss_growth_rate))
...
class Stop_loss_by_28_index(Adjust_condition):
    ...
    def __init__(self, params):
        ...
        self.index8 = params.get('index8', '')
        self.index_growth_rate = params.get('index_growth_rate', 0.01)
        self.dst_minute_count_28index_drop = params.get(
            'dst_minute_count_28index_drop', 120)
        # 临时参数
        self.t_can_adjust = True
    ...
    def update_params(self, context, params):
        ...
        self.index_growth_rate = params.get(
            'index_growth_rate', self.index_growth_rate)
        self.dst_minute_count_28index_drop = params.get(
            'dst_minute_count_28index_drop', self.dst_minute_count_28index_drop)
    ...
    def handle_data(self, context, data):
        ...
        if gr_index2 <= self.index_growth_rate and gr_index8 <= self.index_growth_rate:
            if (self.minute_count_28index_drop == 0):
                self.log_info(
                    "当前二八指数的20日涨幅同时低于[%.2f%%], %s指数: [%.2f%%], %s指数: [%.2f%%]" %
                    (self.index_growth_rate *
                     100,
                     get_security_info(
                         self.index2).display_name,
                        gr_index2 *
        ...
        if self.minute_count_28index_drop >= self.dst_minute_count_28index_drop:
            if self.minute_count_28index_drop == self.dst_minute_count_28index_drop:
                self.log_info(
                    "==> 当日%s指数和%s指数的20日增幅低于[%.2f%%]已超过%d分钟，执行28指数止损" %
                    (get_security_info(
                        self.index2).display_name,
                        get_security_info(
                        self.index8).display_name,
    ...
    def print_win_rate(self, current_date, print_date, context):
        if str(current_date) == str(print_date):
            ...
            if 0 < self.trade_total_count and 0 < self.trade_success_count:
                win_rate = round(
                    self.trade_success_count /
                    float(
                        self.trade_total_count),
                    3)
            ...
            s = '\n------------绩效报表------------'
            s += '\n交易次数: {0}, 盈利次数: {1}, 胜率: {2}'.format(
                self.trade_total_count, self.trade_success_count,
            ...
            s += '\n总资产: {0}, 本金: {1}, 盈利: {2}, 盈亏比率：{3}%'.format(
                starting_cash + total_profit, starting_cash, total_profit,
...
class Shipane_sync_p(Rule):
    ...
    def __sell(self, client, context):
        ...
        for x in op_list:
            if x[1] > 0:
                continue
            try:
                actual_order = client.execute(
                    self._client_param, action='SELL', symbol=x[0],
        ...
        for x in op_list:
            ...
            try:
                ...
                for i in range(2):
                    ...
                    self.log_info(
                        '%d 次买计算:[stock : %s][max_price:%f] [cash: %f] [max_count:%d] [aim:%d]' %
                    ...
                    if max_count >= buy_count:
                        # 可用资金足够，一次性买入。
                        actual_order = client.execute(
                            self._client_param, action='BUY', symbol=stock,
    ...
    def __get_shipan_p(self, client):
        ...
        sp = [[normalize_code(x[0]).encode('utf-8'), int(float(x[1]))]
              for x in sp if x[0] != '' and x[1] != '']
        return sp
    ...
    def __get_dif(self, mp, sp):
        ...
        s_list = [[s, sum([x[1] for x in op_list if x[0] == s])]
                  for s in s_list]
...
class Purchase_new_stocks(Rule):
    ...
    def __str__(self):
        return '实盘易申购新股[time: %s] ' % (self.times)
...
def is_3_black_crows(stock):
    ...
    # 一阳三阴
    if h_close[-4] > h_open[-4] and (h_close[-1] < h_open[-1]
                                     and h_close[-2] < h_open[-2] and h_close[-3] < h_open[-3]):
