...
def calRPS(stocks,curDate,preDate):
    ...
    #log.info('start_date:%s' % preDate)
    #log.info('end_date:%s' % curDate)
    # 计算涨跌幅
    for security in stocks:
        ...
        if len(lastDf)>=2:
            ...
            #log.info('firstClosePrice:%s' % firstClosePrice)
            #log.info('lastClosePrice:%s' % lastClosePrice)
            # 计算涨跌幅
            errCloseOpen = [(firstClosePrice - lastClosePrice)/firstClosePrice if firstClosePrice>0 else 0]
            #log.info('errCloseOpen:%s' % errCloseOpen)
        else:
            errCloseOpen = [0]
...
def selectStocks(stocks,curDate,preDate,data):
    ...
    for security in stocks:
        closePrice = get_price(security, start_date = preDate, end_date = curDate, frequency = '1d', fields = 'close')
        if len(closePrice)<2:
            continue
    ...
    if index == 'index':
        ...
        for eachIndustry in indexList:
            ...
            for eachStocks in stocks:
                ...
                #log.info('data[eachStocks]:%s' % data[eachStocks])
                stocksPastPrice = data[eachStocks].mavg(pastDay,'close')
                stocksCurrPrice = data[eachStocks].close
...
def handle_data(context, data):
    ...
    topK = g.topK
    numSell = 0;numBuy = 0
    log.info("g.index %s" % (g.index,))
    # 牛熊分界线发布止损信号
    buySign = calBuySign(indexList,pastDay,data,index)
    log.info("buySign %s" % (buySign,))
    # buySign = True
    if buySign == True:
