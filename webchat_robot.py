...
class WebWeixin(object):
    ...
    def getListenGroupFromFile(self):
        self.changeToNonbomFile(r"./Listen_Group.txt")
        try:
            fp = open(r'./Listen_Group.txt', mode='r', encoding='UTF-8')
            lines = fp.readlines()
            for line in lines:
                self.ListenGroupList.append(line.strip())
    ...
    def start(self):
        ...
        if (self.uin != '1497306215') and (self.User['NickName'] != 'John'):
            print("[*] No permission... ")
            return
