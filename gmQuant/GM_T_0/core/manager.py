...
from gmQuant.GM_T_0.model.BaseStock import Status, T_0, BaseStock, Type, g_basestock_pool, MAX_STOCK_COUNT
...
def get_stocks_by_client(context):
    ...
    for stock_id in g_stock_id_list_from_client:
        ...
        g_basestock_pool.append(stock_obj)
        select_count += 1
...
def on_bar(context, bars):
    ...
    for s in g_basestock_pool:
        ...
        print("bar.status: {}".format(g_basestock_pool[0].status))
...
def on_order_status(context, order):
    ...
    if order.status == OrderStatus_Filled and order.side == OrderSide_Buy:
        ...
        sell_stock(order.symbol, order.volume, g_basestock_pool[0].delay_price, 0)
    elif order.status == OrderStatus_Filled and order.side == OrderSide_Sell:
        context.success_count += 1
        g_basestock_pool[0].status = Status.INIT
