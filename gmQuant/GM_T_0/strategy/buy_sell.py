...
from gmQuant.GM_T_0.model.BaseStock import Type, Status, g_basestock_pool
...
def buy_stock(stock, amount, limit_price, index):
    ...
    if buy_order is not None:
        g_basestock_pool[index].buy_price = limit_price
        # print("买入股票: %s, 以%f价格挂单，买入%d, 成功" % (stock, limit_price, amount))
        return True
    else:
        g_basestock_pool[index].buy_price = 0
...
def sell_stock(stock, amount, limit_price, index):
    ...
    if sell_order is not None:
        g_basestock_pool[index].sell_price = limit_price
        #print("卖出股票: %s, 以%f价格挂单，卖出%d" % (stock, limit_price, amount))
        return True
    else:
        g_basestock_pool[index].sell_price = 0
        return False


def sell_signal(context, stock, close_price, index):
    if g_basestock_pool[index].status == Status.WORKING:
        # print(" 股票: %s, 收到重复卖出信号，但不做交易" % stock)
        return

    # 每次交易量为持仓量的g.adjust_scale
    amount = context.adjust_scale * g_basestock_pool[index].position
    ...
    print("卖出信号: %f, src_position: %d, amount: %d, price: %f" %
          (context.adjust_scale, g_basestock_pool.position, amount, limit_price))

    if g_basestock_pool[index].status == Status.INIT:
        ...
        g_basestock_pool[index].t_0_type = Type.Active_Sell
        g_basestock_pool[index].delay_amount = amount
        g_basestock_pool[index].delay_price = limit_price
        g_basestock_pool[index].status = Status.WORKING  # 更新交易状态
    else:
        print("股票: %s, 交易状态出错" % stock)


def buy_signal(context, stock, close_price, index):
    print(g_basestock_pool[index].status)
    if g_basestock_pool[index].status == Status.WORKING:
        print("%s 股票: %s, index: %d 收到重复买入信号，但不做交易" % (context.now, stock, index))
        return

    # 每次交易量为持仓量的g.adjust_scale
    amount = floor(context.adjust_scale * g_basestock_pool[index].position)
    ...
    print("买入信号: %f, src_position: %d, amount: %d, price: %f" %
          (context.adjust_scale, g_basestock_pool[index].position, amount, limit_price))
    ...
    if g_basestock_pool[index].status == Status.INIT:
        ...
        yesterday = history(symbol=stock, frequency='1d', start_time=context.lastday,
                            end_time=context.today, fields='close', df=True)
        limit_price = close_price + yesterday.iat[0, 0] * context.expected_revenue
        g_basestock_pool[index].t_0_type = Type.Active_Buy
        g_basestock_pool[index].delay_amount = -amount
        g_basestock_pool[index].delay_price = limit_price
        g_basestock_pool[index].status = Status.WORKING  # 更新交易状态
        g_basestock_pool[index].start_time = context.now
